...
export function ModulesList() {
  ...
  return (
    <div style={{ paddingBottom: 48 }}>
      ...
      <p style={{ color: 'var(--text-secondary)', fontSize: 14, margin: '0 0 32px' }}>Formaciones y módulos disponibles.</p>

      {formaciones.length === 0 && (
        <div style={{ color: 'var(--text-muted)', textAlign: 'center', padding: 40 }}>Todavía no hay formaciones cargadas.</div>
      )}
        ...
        return (
          <div key={formacion.id} style={{ marginBottom: 40 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              {!unlocked && (
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', border: '1px solid rgba(var(--overlay-rgb),0.1)', borderRadius: 99, padding: '2px 10px' }}>
                  Bloqueada
                </span>
              )}
            </div>
            {unlocked ? (
              formModules.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: 13, marginLeft: 26 }}>Sin módulos todavía.</p>
              ) : (
                  ...
                  {formModules.map((mod) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseEnter={(e) => { if (modUnlocked) e.currentTarget.style.borderColor = 'rgba(var(--accent-rgb),0.3)' }}
                      ...
                      >
                        ...
                        <div style={{ padding: 16 }}>
                          ...
                          {mod.description && <p style={{ color: 'var(--text-secondary)', fontSize: 13, margin: '0 0 12px' }}>{mod.description}</p>}
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>
                            <span>Progreso</span><span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{pct}%</span>
                          </div>
                          <div style={{ height: 6, background: 'rgba(var(--overlay-rgb),0.06)', borderRadius: 99, overflow: 'hidden' }}>
                            <div style={{ height: '100%', width: `${pct}%`, background: 'var(--accent)', transition: 'width 0.3s' }} />
                          </div>
                          {pct === 100 && <div style={{ marginTop: 8, fontSize: 12, color: 'var(--success)', fontWeight: 700 }}>✓ Completado</div>}
                        </div>
                      </div>
                    )
                  })}
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 13, marginLeft: 26 }}>Desbloqueala desde "Gestionar equipo".</p>
            )}
          </div>
        )
      })}
